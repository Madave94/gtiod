...
data = dict(
    train=dict(  # Train dataset configs
        ann_file="ablation_studies/ablation50_641x2_640x1_d_train.json",
        ann_pre_processing_fn=dict(
            new_ann_path="texbig_train_laem_union_ablation50_641x2_640x1_d_train.json",  # Path of annotation file
        ),
    )
)
...
optimizer = dict(  # Config used to build optimizer, support all the optimizers in PyTorch whose arguments are also the same as those in PyTorch
    ...
    lr=0.015,  # Learning rate of optimizers, see detail usages of the parameters in the documentation of PyTorch
...
work_dir = 'logs/texbig_detectoRS_ablation50_641x2_d'  # Directory to save the model checkpoints and logs for the current experiments.
